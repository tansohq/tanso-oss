...
export function PublishWeightsDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Publish tariff</DialogTitle>
          <DialogDescription>
            All changes below take effect together at the chosen time. Charges before that time
        ...
        <div className="flex flex-col gap-4">
          <ul className="flex flex-col gap-1 rounded-lg border bg-muted/50 p-3 font-mono text-sm">
            {changes.map((change) => (
              <li key={`${change.featureId}|${change.model ?? ""}`}>
                {change.featureKey}
                {change.model ? ` (${change.model})` : ""}: {change.from ?? "default"} →{" "}
                {change.to}
              </li>
            ))}
          </ul>
          ...
          {isSoon && (
            <Field>
              <FieldLabel htmlFor="publish-confirm">
                This tariff takes effect in under an hour. Type{" "}
