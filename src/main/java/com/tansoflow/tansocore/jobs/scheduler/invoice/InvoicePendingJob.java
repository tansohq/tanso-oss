...
public class InvoicePendingJob {
    ...
    @Scheduled(cron = "${jobs.invoicePending.cron}")
    @SchedulerLock(name = "invoicePendingJob", lockAtMostFor = "PT10M", lockAtLeastFor = "PT1M")
    @Transactional
    public void run() {
        log.info("Running Invoice Pending Job");
