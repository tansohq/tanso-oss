...
*   **Gateway mode (phase 5)**: `VendorProvider.LITELLM` — the connection's `scope` is the proxy URL and `admin_key` its master key (`LiteLlmUsagePuller.base()` insists on http(s)). Pull: `GET /spend/logs?start_date&end_date&summarize=false`, rows aggregated per (day, model, `team_id`→workspaceId, `api_key`→vendorApiKeyId, `end_user`|`user`→actorId) into a USAGE_API row (prompt/completion tokens, request count) + a COST_API row (`spend` dollars → cents, description "LiteLLM spend via <custom_llm_provider>"). Push: `GatewayEnforcementServiceImpl.apply(budget)` runs inside `putBudget`/`deleteBudget` — for every LITELLM connection × every LITELLM rule on the unit, `LiteLlmGateway.pushMonthlyBudget` POSTs `/team/update` (WORKSPACE_ID), `/key/update` (API_KEY_ID) or `/user/update` (ACTOR) with `max_budget` (dollars) + `budget_duration: "1mo"`; a non-Block budget (or delete) pushes `max_budget: null` to clear. Outcome lands on `spend_budgets.enforced_at/enforcement_target/enforcement_error` (changelog `2026.08.25.8`) and in `SpendBudgetDto`; the breach alert says "Enforced at litellm:team:…" instead of "cannot block". Never throws — a gateway being down must not block the save. Daily ceilings stay advisory (LiteLLM has one duration per object).
*   **Alerts to standard (phase 6)**: `SpendNotifier.notify(account, event, subject, text, html, payload)` fans out to Slack (`SlackNotifier`), a generic webhook (`external_api_keys` types `SPEND_WEBHOOK` + `SPEND_WEBHOOK_SECRET`, entity `WEBHOOK`; body `{type, accountId, alert|digest}`, headers `X-Tanso-Event`, `X-Tanso-Signature: sha256=<hex HMAC-SHA256(secret, body)>`) and email via the existing `Resend` bean (`account_settings.spend_alert_emails`, sender `app.spend.alert-from`); every channel catches + logs, never throws. `SpendAlertKind.PROJECTED`: in `evaluate`, once per month window when ≥ 1/5 of the month has elapsed, spent < limit, and spent × total/elapsed > limit. Bump: `spend_budgets.bump_monthly_cents/bump_expires_at/bump_reason` (changelog `2026.08.25.9`), `SpendBudget.effectiveMonthlyCents(now)` is what `evaluate`, the gateway push and the digest use; `POST/DELETE /units/{id}/budget/bump`; an expired bump is cleared inside `evaluate` (re-applies the gateway, saves). Digest: `SpendDigestServiceImpl.build` = last 7 full UTC days vs the 7 before per unit (allocation `totalCents`), month-to-date vs effective limit, alerts fired in the week; `send` posts `spend.digest`; `GET /digest`, `POST /digest/send`; `SpendDigestJob` Monday 08:00 UTC (`jobs.spendDigest.cron`) for `account_settings.spend_digest_enabled`. Console: settings sheet (emails, digest switch, webhook + secret), Alerts page digest card, budget card bump form.
*   **Savings + route simulator (phase 7)**: `SpendSavingsServiceImpl` over USAGE_API buckets only. `savings`: per (provider, model) input-side cents as billed (`inputSideCents`: uncached@input + read@cacheRead|input + write@cacheWrite|input) vs `noCache` (all input tokens @ input rate); `savedCents = noCache − billed` (negative when writes outweigh reads); `cacheShare = read/(uncached+read+write)`; unpriced models listed, `cacheRatesKnown=false` rows read as zero saving. `simulate(fromModel, toModel, workspaceId?)`: sums matching buckets' tokens, prices both models via `ModelPricingResolver` (target must resolve → else 400), `deltaCents = simulated − current`, `caveats[]` always carries tokenizer + quality lines plus fuzzy-match / missing-cache-rate / empty-traffic notes. `GET /reports/savings`, `POST /reports/simulate`, `GET /reports/models` (price book). Console: Internal spend → Savings. Never a router — decision from the 2026-08-25 review.
*   **Feature P&L (phase 8)**: `spend_units.feature_id` (changelog `2026.08.25.10`; PROJECT only, validated against `FeatureRepository.findByIdAndAccountId`, set via `SpendUnitRequest.featureId`). `SpendPnlServiceImpl.report` = per linked project: `buildCents` (allocation `totalCents`, descendants included), `outcomes` (outcome report), serve side from `EventRepository.sumRevenueAndCostByFeature` (CLIENT_TRACKED + ENTITLEMENT_CHECKED, `revenueAmount`/`costAmount` dollars → cents), `serveMargin = revenue − serveCost`, `net = serveMargin − build`, `buildPerOutcome`; `unlinkedProjects` names PROJECT units without a feature (their build cost stays out of the totals). `GET /reports/pnl`. Console: Internal spend → P&L; unit form shows a Feature picker for projects.
*   **Review fixes (2026-08-25, on the stack tip)**: `OutboundUrlPolicy` (`util`) gates every operator-typed URL the server calls — LiteLLM scope at connection create, generic webhook at settings save: http(s) only, no userinfo, host must resolve, loopback/link-local/unspecified/multicast refused, RFC1918 refused only when `app.spend.outbound.allow-private=false`; LiteLLM error messages echo only a JSON error field (`VendorErrors.jsonMessage`), never a raw body. `VendorSyncServiceImpl`: vendor HTTP runs outside any transaction, one `pullAll` pass per chunk (Copilot fetches each day once, no cache), delete+rewrite under `pg_advisory_xact_lock(hashtext(connectionId))`, rows outside the window dropped+logged, an empty pull over a window that had rows throws instead of wiping it, every `RuntimeException` marks the connection ERROR and the loop continues; `SpendBudgetServiceImpl.evaluate` takes `pg_advisory_xact_lock` per account so once-per-window is race-free. Slack/webhook failures log class + status only (the URL is the secret). Outcome `url` must be http(s). P&L: revenue/cost only where the unit is CURRENCY/null; a linked project under a linked project is not double counted in totals. Claude Code CLAUDE_CODE_API rows carry no session count; actor metrics fold per (day, actor). `spend_budgets.enforcement_target` / `spend_alerts.message` are TEXT (changelog `2026.08.25.11`). Console: `useUpdateSpendUnit` sends every field (it used to drop `featureId`/`githubLogin` → cleared on rename); delete budget removes the query; rules/settings invalidate P&L, outcomes, digest; list pages render server errors instead of an empty table.
