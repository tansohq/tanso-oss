<template>
  <!-- Mobile gate -->
  <div
    v-if="isMobile && showNav"
    class="flex min-h-svh flex-col items-center justify-center px-6 text-center bg-background"
  >
  ...
  <TooltipProvider v-else>
    ...
    <template v-if="isReady">
      ...
      <SidebarProvider v-else-if="showNav">
        ...
        <SidebarInset>
          <header
            class="flex h-14 items-center gap-2 px-3 border-b md:hidden bg-sidebar text-sidebar-foreground"
          >
            <SidebarTrigger />
          </header>
          <RouterView />
